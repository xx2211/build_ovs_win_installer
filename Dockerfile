...
# Note: Time-consuming operation contains: Download, install choco, install app by choco, install MSBuilds
...
COPY "./res/*.msm" "C:/Program Files (x86)/Common Files/Merge Modules/*.msm"
...
RUN c:\BuildTools\VC\Auxiliary\Build\vcvarsall.bat x64 10.0.18362.0 `
    ...
    "cd /c/TEMP/openvswitch-3.1.2/ && ./boot.sh && make -j8 && make windows_installer"

CMD ["echo", "Please installer and driver to host machine.",`
    "&&", "echo", "Installer: xxx",`
    "&&", "echo", "driver: xxx",`
    "&&", "pause"]
